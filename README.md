...
- 拖入非 CSV 文件时提示「仅支持 CSV 文件」；混合拖拽时会提示忽略的非 CSV 文件数量
- 全局拖拽保护：文件拖到图表区域外时不会触发浏览器导航（避免丢失内存中数据）
...
- 释放缓冲从 3s 延长至 7s，确保每次循环 CSV 数据完整分离
- 压力上升等待超时从 30s 缩短至 15s，超时弹出 toast 警告"压力机未正确压到压力计上"
- 修复缓冲区 `isRunning` 翻转导致的 3 个状态 bug：
  - 第 2+ 轮热身期意外采集数据
  - 缓冲期间点击中止后 `isRunning` 卡死为 true
  - 循环正常完成后 `pressMode` 不复位，按钮停留在"停止"状态
- 每段 `sleepWithAbort` 后增加中止标志检查，确保即时响应停止操作

**修改文件：**
- 修改 `client/src/pages/ConsistencyPage.tsx` — 缓冲逻辑重写 + 超时警告 + 状态修复
- 修改 `client/src/version.ts` — 版本号更新为 v1.9.29

### v1.9.28（2026-07-23）

**Feat：下压机改为压力事件驱动模式**

- 下压节奏不再依赖固定 10s 定时器，改为实时监测压力计数据：
  - 发送下压指令 → 等待压力 >1N（确认到位）→ 等待压力 <1N（确认释放）→ 缓冲 3s → 下一轮
  - 采集过程同样跟随压力事件：压力到位→释放→缓冲后自动停止导出 CSV
- 安全性增强：
  - 压力计未连接时直接阻止启动并提示错误
  - 压力计数据 null 时立即返回失败而非误判为"已释放"
  - 阈值滞回设计（`>1.0` / `<1.0`），避免精确 1.0N 假触发
  - Arduino 写入失败立即跳过本轮，不空等 30s 超时
  - 所有等待操作均可通过"停止"按钮即时中断

**修改文件：**
- 修改 `client/src/pages/ConsistencyPage.tsx` — 重写 handleStartPress 为压力事件驱动
- 修改 `client/src/version.ts` — 版本号更新为 v1.9.28
...
- 登录页面：启动时显示登录表单，用户名+密码验证
- 客户账号配置白名单控制传感器产品可见范围
- 不同客户登录后看到不同的传感器下拉选项
- 登录状态可持久化（记住登录），右上角显示当前客户名 + 退出按钮
...
**新增传感器产品：定制传感器 (16×16 单帧协议)**

适配定制传感器的通信协议：

- 单帧协议 278 字节：帧头(4B) + 无效数据(2B) + 传感器(256B) + 陀螺仪(16B)
- 16×16 矩阵，256 个 ADC 点
- 协议通过独立解析分支处理

**修改文件：**

- 修改 `client/src/hooks/useSerialPort.ts` — 新增定制协议解析
- 修改 `client/src/components/SerialConnectPanel.tsx` — 产品列表新增定制传感器
